...
import { AlertDialog, cactusToMojo } from '@cactus/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      cactusper_input.value === '' ||
      Number(cactusper_input.value) === 0 ||
      !Number(cactusper_input.value) ||
      isNaN(Number(cactusper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const cactusper = cactusToMojo(cactusper_input.value);
    const cactusper_value = Number.parseInt(Number(cactusper));
    ...
    // var fee = cactusToMojo(fee_input.value);
